        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                try {
                    ...
                    const md = "SANDEEP X MD~" + base64Session;
...
`;
                ...
                await delay(10000);
                SANDEEP X MD_PAIR_CODE(id, num, res);
            }
        });
    } catch (error) {
        logger.error(`Error in SANDEEP X MD_PAIR_CODE: ${error.message}`);
        removeFile(path.join(__dirname, 'temp', id));
        if (!res.headersSent) {
...
router.get('/', async (req, res) => {
    ...
    await SANDEEP X MD_PAIR_CODE(id, num, res);
});
